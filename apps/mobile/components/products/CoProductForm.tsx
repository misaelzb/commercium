...
import { productDataAsForm, productFormApiParse, ProductInputData } from "@/util"
...
export const CoProductForm = ({ type, initialProduct }: CoProductFormProps) => {
    ...
    const [data, setData] = useState<ProductInputData>(initialProduct ? productDataAsForm(initialProduct) : {
        sku: '',
        description: '',
        costPrice: '',
        salePrice: '',
    ...
    const handleSubmit = async () => {
        ...
        if (json.error) {
            let newErrors = json.zodIssues?.map((x: { path: any[]; message: any }) => {
    ...
    return <CoCard>
        ...
        {errors.global && <CoText style={{ color: Palette.danger, textAlign: 'center' }}>{errors.global}</CoText>}
        {successMsg && <CoText style={{ color: Palette.success, textAlign: 'center' }}>{successMsg}</CoText>}
        <CoSeparator />
        <CoInput
            label="SKU"
        ...
        <CoInput
            ...
            placeholder="E.g: 100"
            keyboardType="numeric"
            onChangeText={(value) => updateInput('stock', value)}
            value={data.stock}
            error={errors.stock} />
