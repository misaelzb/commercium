...
export namespace Products {
    ...
    export const update = async (storeId: number, originalSku: string, data: z.infer<typeof ProductUpdateSchema>): Promise<DBQueryResponse> => {
        ...
        if (data.sku && data.sku !== product.sku) {
            ...
            if (existingProduct) {
                return { success: false, errorDetail: "SKU is already in use by another product" };
            }
        }
    ...
    export const listAll = async (storeId: number): Promise<typeof productsTable.$inferSelect[]> => {
        let products = await Drizzle.db.select().from(productsTable).where(eq(productsTable.storeId, storeId));
        return products;
    }

    export const remove = async (storeId: number, sku: string): Promise<DBQueryResponse> => {
        let product = await fetch(storeId, sku);
        if (!product) return { success: false, errorDetail: "Product not found" };

        await Drizzle.db.delete(productsTable).where(eq(productsTable.id, product.id));
